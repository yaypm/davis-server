...
class Dynatrace {
     ...
     * @param {String} [status=null] - The status of problems (open or closed)
     * @returns {Promise}
     */
    getFilteredProblems(timeRange, applications, status) {
    ...
    /**
     * Returns a filtered list of timeseries data points
     * @param {Object} [timeRange=null] - The time range generated by generateTimeRange
     * @param {(string|string[])} [applications=null] - An application or array of applications
     * @param {Object} [timeseriesOptions=null] - Time series options
     * @returns {Promise}
     */ 
    getFilteredTimeseries(timeRange, applications, timeseriesOptions) {
        return new BbPromise((resolve, reject) => {

            if (_.isNil(timeRange)) {
                logger.info('The user is interested in timeseries data up until now');

                rp(this.options('timeseries', { timeseriesId: timeseriesOptions.timeseriesId, relativeTime: 'day', queryMode: 'series', aggregationType: timeseriesOptions.aggregationType }))
                    .then( response => {
                        resolve(response);
                    })
                    .catch( err => {
                ...
                logger.info('The user has requested timeseries data from a specific timeframe');
                
                let relativeTime = getTimeFilter(timeRange.startTime).relativeTime;
                rp(this.options('timeseries', { timeseriesId: timeseriesOptions.timeseriesId, relativeTime: relativeTime, queryMode: 'series', aggregationType: timeseriesOptions.aggregationType }))
                    .then( response => {
                        resolve(response);
                    })
                    .catch( err => {
