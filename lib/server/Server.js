  ...
  constructor(davis) {
    ...
    app.use((req, res, next) => {
      if (!req.url.includes('/oauth') && !req.url.includes('/slack/receive')) {
        return res.sendFile(path.resolve(path.join(__dirname, '../../web/dist/index.html')), err =>
            res
              .status(404)
              .send('It appears you do not have your web/dist folder built. Please install the production tarball or compile the web files.'));
      }
      return next();
    });
  }
