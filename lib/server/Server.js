...
class Server {
  ...
  manageSocketConnections() {
    this.io.sockets.on('connection', socket => {
      ...
      this.activeSocketConnections.push({ socket, userId: null });
      ...
      socket.on('registerSocket', data => {
        ...
        this.activeSocketConnections.forEach((connection, index) => {
          if (connection.socket.id.replace('/#', '') === data.id) {
            ...
            this.davis.users.verifyToken(data.token, (err, decoded) => {
              if (err) socket.disconnect('unauthorized');
              this.activeSocketConnections[index].userId = data.userId;
            });
          }
        });
      });
  ...
  pushLinkToUser(user, link) {
    this.io.sockets.emit(`url-${user.email}`, link);
  }
    ...
    return new BbPromise.resolve().then(() => {
      ...
      this.app.use((req, res, next) => {
        ...
        if (process.env.NODE_ENV === 'production' && process.platform === 'win32') {
          ...
          req.status(401).json({ status: 'failure', reason: 'cannot validate Alexa requests on win32' });
          return;
        }
