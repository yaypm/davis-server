...
class Server {
    ...
    this.io.sockets.on('connection', socket => {
      ...
      socket.on('registerSocket', data => {
        this.davis.logger.debug(
          `registerSocket with socket id: ${data.id}`
        );
  ...
  pushLinkToUser(user, link) {
    let token = this.getChromeToken(user);
    if (token) {
  ...
  start() {
    return new BbPromise.resolve().then(() => {
      ...
      this.app.use('/', routes);
      this.app.use('/node_modules', express.static(path.join(__dirname, '../../web/node_modules')));
      this.app.use(express.static(path.join(__dirname, '../../web/src')));
      
      this.app.get('*',function(req, res) {
        res.sendFile(path.resolve(path.join(__dirname, '../../web/src/index.html')));
      });
