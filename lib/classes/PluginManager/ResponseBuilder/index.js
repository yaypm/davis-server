...
class ResponseBuilder {
  ...
  createSlackCard(text) {
    return { attachments: [{ text, fallback: text }] };
  }
  ...
  getTemplates(plugin, tagDir) {
    try {
      ...
      const relTemplates = _.mapValues(templates, t => path.relative(this.davis.dir, t));
      return relTemplates;
    } catch (err) {
  ...
  build(exchange) {
    ...
    if (exchange.noResponse) return null;
    const out = {};
    let dir;
    const context = exchange.getExchangeContext();
    /*
    {
    ...
    */
    ...
    try {
      ...
      if (!out.say && !out.text) {
        throw new this.davis.classes.Error('Must set at least a say or text response');
      }

      // ensure we are working with a VB.Message
      if (out.show && !(out.show.constructor === VB.Message)) {
        out.show = VB.Message.fromSlack(out.show);
      }

      let base = out.text || out.say;
      ...
      if (!_.has(out, 'show')) out.show = new VB.Message().addText(base);
      ...
      if (exchange.shouldGreet()) {
        ...
        if (!_.isNil(greeting)) {
          ...
          out.show.setGreeting(greeting);
        }
      }

      if (!exchange.shouldConversationEnd() && exchange.shouldFollowUp()) {
        let followUp = exchange.getFollowUpQuestion();

        if (followUp === '') {
          // automatic follow up
          exchange.addContext({
            targetIntent: 'autoFollowUp',
            previousTarget: exchange.getContext().targetIntent,
          });
          followUp = _.sample([
        ...
        out.show.setFollowUp(followUp);
      }

      // Combine multiple lines into one, ensuring double spaces after periods.
      out.say = out.say.split(/\s+/).join(' ').split('. ').join('.  ');
      out.text = out.text.split(/\s+/).join(' ').split('. ').join('.  ');

      out.say = this.pronounceable(out.say);

      // sources are not yet ready to receive a VB.Message object
