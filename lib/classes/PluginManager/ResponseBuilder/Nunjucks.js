...
function friendlyTimeRange(timeRange, user, isCompact, displayType) {
  ...
  const type = displayType || 'audible';
  const timezone = (type === 'visual') ? moment.tz(user.timezone).format('z') : '';
  ...
  if (isCompact) {
    sentence = `${mStart.calendar(null, startFormat.normal).trim() } ${ timezone}`;
    ...
    if (moment().isBefore(mStop)) {
      sentence += ` - ${now.calendar(null, stopFormat.normal)}`;
    } else if (moment.duration(mStop.diff(mStart, 'hours')) < 24) {
      sentence += ' - ';
      sentence += `${capitalizeFirstCharacter(mStop.calendar(null, stopFormat.sameday)).trim()} ${timezone}`;
    } else {
      sentence += ' - \\n';
      sentence += `${capitalizeFirstCharacter(mStop.calendar(null, stopFormat.normal)).trim()} ${timezone}`
    }
  } else {
    sentence = `${mStart.calendar(null, startFormat.between).trim() } ${timezone}`.trim();
    ...
    if (moment().isBefore(mStop)) {
      ...
      sentence += `${mStop.calendar(null, stopFormat.sameday).trim() } ${timezone}`;
    } else {
      sentence += `${mStop.calendar(null, stopFormat.normal).trim()} ${timezone}`;
    }
  }
