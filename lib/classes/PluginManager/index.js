...
class PluginManager {
  ...
  loadPlugins(plugins, basedir) {
    let n = 0;
    _.forEach(plugins, (plugin) => {
      try {
        const Plugin = require(plugin); // eslint-disable-line
        Plugin.dir = path.join(path.relative(basedir, plugin));
  ...
  loadCorePlugins() {
    ...
    const n = this.loadPlugins(coreIntents, this.davis.dir);
  ...
  loadUserPlugins(plugins) {
    const userIntents = plugins
      .map(userIntentPath => path.join(this.davis.dir, userIntentPath));

    const n = this.loadPlugins(userIntents, process.cwd());
  ...
  _runIntent(intent, exchange) {
    ...
    return BbPromise.reduce(hooks, (__, hook) => {
      ...
      .then(() => {
        ...
        if (intent.key === 'routing') return exchange.save();
