...
class PluginManager {
  ...
  _setApplicationEntities(applications) {
    ...
    return BbPromise.map(applications, app =>
  ...
  _setServiceEntities(services) {
    return BbPromise.map(services, serv =>
      AliasesModel.findOne({ entityId: serv.entityId })
        .exec()
        .then((alias) => {
          if (!_.isNil(alias)) return alias;
          const name = serv.customizedName || serv.displayName;
          return new AliasesModel({
            name,
            category: 'services',
            entityId: serv.entityId,
            display: {
              visual: name,
              audible: name,
            },
