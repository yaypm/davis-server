...
class Nlp {
  ...
  parseTimeValue(datetime) {
    ...
    switch (datetime.grain) {
      ...
      case 'minute':
        timeRange = {
          startTime: moment(datetime.value).subtract(5, 'minutes').valueOf(),
          stopTime: moment(datetime.value).add(5, 'minutes').valueOf(),
        };
        break;
      case 'hour':
        timeRange = {
          startTime: moment(datetime.value).subtract(15, 'minutes').valueOf(),
          stopTime: moment(datetime.value).add(15, 'minutes').valueOf(),
        };
        break;
      case 'day':
        timeRange = {
          startTime: moment(datetime.value).startOf('day').valueOf(),
          stopTime: moment(datetime.value).endOf('day').valueOf(),
        };
        break;
      case 'week':
        timeRange = {
          startTime: moment(datetime.value).startOf('week').valueOf(),
          stopTime: moment(datetime.value).endOf('week').valueOf(),
        };
        break;
      case 'month':
        timeRange = {
          startTime: moment(datetime.value).startOf('month').valueOf(),
          stopTime: moment(datetime.value).endOf('month').valueOf(),
        };
        break;
      case 'year':
        timeRange = {
          startTime: moment(datetime.value).startOf('year').valueOf(),
          stopTime: moment(datetime.value).endOf('year').valueOf(),
        };
        break;
      default:
  ...
  parseTimeRange(datetime) {
    ...
    if (datetime.to) {
      timeRange = {
        startTime: moment(datetime.from.value).valueOf(),
        stopTime: moment(datetime.to.value).subtract(1, datetime.to.grain).valueOf(),
      };
    } else {
      timeRange = {
        startTime: moment(datetime.from.value).valueOf(),
      ...
      .then((res) => {
        ...
        if (datetime.type) {
          if (datetime.type === 'value') {
            ...
            this.logger.debug(`found: ${moment(timeRange.startTime).format()} to ${moment(timeRange.stopTime).format()}.`);
   ...
   */
  classify(text) {
    if (!this.trained) throw new this.pluginManager.davis.classes.Error('Cannot classify before trainging');
