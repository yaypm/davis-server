  ...
  parseTimeValue(datetime, timezone) {
    ...
    switch (datetime.grain) {
      ...
      case 'year':
        ...
        break;
      default:
        this.pluginManager.davis.logger.error(`Passed in an unknown granularity: ${datetime.grain}`);
    }
    return timeRange;
  }
  ...
  parseTimeRange(datetime, timezone) {
    ...
    if (datetime.to) {
      timeRange = {
        ...
        stopTime: moment(datetime.to.value).tz(timezone),
      };
    } else {
      timeRange = {
        startTime: moment(datetime.from.value).tz(timezone),
      ...
      .then((res) => {
        ...
        if (datetime.type) {
          if (datetime.type === 'value') {
            this.pluginManager.davis.logger.debug('parsing time value');
            timeRange = this.parseTimeValue(datetime, tz);
            this.pluginManager.davis.logger.debug(`found: ${timeRange.startTime.format()} to ${timeRange.stopTime.format()}.`);
          } else if (datetime.type === 'interval') {
            this.pluginManager.davis.logger.debug('parsing time interval');
            timeRange = this.parseTimeRange(datetime, tz);
            this.pluginManager.davis.logger.debug(`found: ${timeRange.startTime.format()} to ${timeRange.stopTime.format()}.`);
          } else {
            this.pluginManager.davis.logger.error(`Unknown datetime type ${datetime.type}.`);
          }
        }
