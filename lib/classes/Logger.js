...
class Logger {
  ...
  asciiGreeting() {
    ...
    this.console(' ');
    this.console(chalk.yellow('  Your Environment Information ----------------------------'));
    this.console(chalk.yellow(`     OS:                 ${process.platform}`));
    this.console(chalk.yellow(`     Node Version:       ${process.version.replace(/^[v|V]/, '')}`));
    this.console(chalk.yellow(`     Davis Version:      ${version}`));
  }
