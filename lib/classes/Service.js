...
class Service {
  ...
  */
  ...
  updateAlias(_id, audible, visual, aliases) {
    return BbPromise.resolve([
      AliasesModel.findById({ _id }),
      AliasesModel.find({ _id: { $ne: _id }, aliases: { $in: aliases } }),
    ])
      .spread((alias, others) => {
        if (others.length > 0) {
          const intersect = _.intersection(others[0].aliases, aliases);
          throw new this.davis.classes.Error(`The alias '${intersect[0]}' already refers to ${others[0].name}`);
        }
