  ...
  */
  connectToMongoDB() {
    return new BbPromise((resolve, reject) => {
      this.logger.debug('Attempting to connection to MongoDB.');
      try {
        mongoose.connect(this.config.getMongoDBConnectionString(), err => {
          if (err) throw new DError('Unable to connect to MongoDB!');
          this.logger.debug('Successfully connected to MongoDB.');
          resolve();
        });
      } catch (e) {
        reject(e);
          ...
          .then(res => {
            ...
            if (!_.isNull(res)) {
              if (res.State === problem.State) {
                throw new DError('A problem with that ID already exists.');
              }
  ...
  getTtsToken() {
    return new BbPromise((resolve, reject) => {
      if (this.config.isWatsonEnabled()) {
        ...
        reject(new DError('Watson TTS service is disabled!'));
      }
    });
  }
  ...
  getSttToken() {
    return new BbPromise((resolve, reject) => {
      if (this.config.isWatsonEnabled()) {
        ...
        reject(new DError('Watson STT service is disabled!'));
      }
    });
  }
