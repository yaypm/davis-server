...
class Users {
  ...
  authenticateUser(email, password, routerToken) {
    return BbPromise.try(() => {
      ...
      .then((user) => {
        ...
        // create a token
        const token = (routerToken) ? this.createRouterToken(user) : this.createJwtToken(user);
  ...
  createRouterToken(user) {
    user.routerToken = rand.generate(32);

    return user.save()
      .then(() => user.routerToken);
  }
  ...
  verifyRouterToken(token) {
    return UserModel.findOne({ routerToken: token }).select('email admin').exec()
      .then((user) => {
        if (!user) {
