...
class Dynatrace {
  ...
  getFilteredProblems(exchange) {
    ...
    if (_.isNil(timerange)) {
      ...
      filter.relativeTime = this._getTimeFilter(timerange.startTime, timerange.stopTime);
    }
   ...
   *
   */
  _getTimeFilter(start, current) {
    const startTime = moment(start) || moment().subtract(10, 'minutes');
    const currentTime = moment(current) || moment();
  ...
  _filterProblemsByApps(app, res) {
    // No need to filter if an app name wasn't' provided
    if (_.isNil(app)) return res;

    // TODO: Filter the problems by app name
    return res;
  }
