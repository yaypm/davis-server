...
class Dynatrace {
  ...
  _filterProblemsByTime(timerange, problems) {
    ...
    const filteredProblems = _.filter(problems, problem =>
      this._isProblemInRange(
        timerange.startTime,
        timerange.stopTime,
        problem.startTime,
        problem.endTime));
  ...
  _filterProblemsByDavisFilters(exchange, problems) {
    return this.davis.filters.getFilterByScope(exchange.getScope())
      .then((filter) => {
        if (!_.isNull(filter)) {
          this.logger.debug(`Filtering problems using the '${filter.name}' filter.`);

          const filteredProblems = _.filter(problems, (problem) => {
            let impacted = false;
            const impacts = _.filter(problem.rankedImpacts, impact => problem.impactLevel === 'APPLICATION' && _.includes(filter.applicationIds, impact.entityId));
            if (impacts.length !== 0) {
              impacted = true;
            }
            return impacted;
          ...
          return filteredProblems;
          // this.logger.debug(JSON.stringify(problems));
          /* const filteredProblems = _.filter(problems, (problem) => {
            if (filter.status.length !== 0) {
              if (!_.includes(filter.status, problem.status.toLowerCase())) {
                this.logger.info('filtering based on status');
                return true;
              }
            }

            if (filter.impact.length !== 0) {
              if (!_.includes(filter.impact, problem.impactLevel.toLowerCase())) {
                this.logger.info('filtering based on impact');
                return true;
              }
            }
            this.logger.info('start');
            let shouldFilter = false;
            const output = _.forEach(problem.rankedImpacts, (impact) => {
              if (filter.applicationIds.length !== 0) {
                if (problem.impactLevel.toLowerCase() === 'application') {
                  if (_.includes(filter.applicationIds, impact.entityId)) {
                    this.logger.info(`filtering based on application ID '${impact.entityId}'.`);
                    shouldFilter = true;
                    // this.logger.info(JSON.stringify(problem));
                    return false;
                  }
                }
              }
            ...
            this.logger.info(`end - ${shouldFilter}`);
            return shouldFilter;
          });
          return filteredProblems;*/
        }
