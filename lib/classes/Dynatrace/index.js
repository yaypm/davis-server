...
class Dynatrace {
  ...
  getFilteredTimeseriesData(exchange, parameters) {
    ...
    .then((result) => {
      const entities = Object.keys(result.result.dataPoints);
      result.result.stats = {};
      entities.forEach((entity) => {
        const stats = this._extract(result.result.dataPoints, entity);
        result.result.stats[entity] = stats;
        if (!result.result.stats.maxMean || result.result.stats.maxMean.value < stats.average) {
          result.result.stats.maxMean = {
            entity,
            value: stats.average,
          };
        }

        if (!result.result.stats.minMean || result.result.stats.minMean.value > stats.average) {
          result.result.stats.minMean = {
            entity,
            value: stats.average,
          };
        }
