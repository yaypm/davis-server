...
class Dynatrace {
  ...
  getFilteredTimeseriesData(exchange, parameters) {
    ...
    if (_.isNil(timerange.startTime)) {
      ...
      _.set(exchange, 'model.request.analysed.timeRange.startTime', moment().subtract(1, 'd'));
      _.set(exchange, 'model.request.analysed.timeRange.stopTime', moment());
    } else {
      filter.startTimestamp = timerange.startTime.valueOf();
      filter.endTimestamp = timerange.stopTime.valueOf();
    }
