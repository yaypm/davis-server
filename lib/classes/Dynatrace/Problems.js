...
class Problems {
  ...
  _noProblems(exchange, dateRange) {
    ...
    if (context.app) {
      ...
      ['Nice, no detected problems affected', alias, dateRange, '.'];
    } else {
      ['Nice, there were no problems detected', dateRange, '.'];
    }
  }
  ...
  _manyProblems(exchange, dateRange) {
    ...
    const sentence = app ?
      [dateRange, 'there were', this.length, 'problems that affected', alias, '.'] :
      [dateRange, 'there were', this.length, 'problems that affected', this._appSummary(exchange), 'and', this._serviceSummary(), '.'];
  ...
  _heavilyAffectedApplications(problems, sentence, names) {
    ...
    const alias = (app) ? new VB.Alias(app) : names[app];
...
Problems.title = function (problem, davis, text) {
  ...
  return [th.toTitleCase(th.humanize(titleEvent.eventType)), 'on', alias];
};
