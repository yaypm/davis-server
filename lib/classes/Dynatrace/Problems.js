...
class Problems {
  ...
  _noProblems(exchange, dateRange) {
    ...
    return (context.app) ?
      ['Nice, no detected problems affected', context.app.display.visual, dateRange, '.'] :
  ...
  _manyProblems(exchange, dateRange) {
    const app = exchange.getContext().app;
    ...
    const sentence = app ?
      [dateRange, 'there were', this.length, 'problems that affected', app.display.visual, '.'] :
