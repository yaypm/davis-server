...
class Exchange {
  ...
  start(request, source, scope) {
    return BbPromise.try(() => {
      if (!_.isString(request)) throw new this.davis.classes.Error('A user request is required!');
    ...
    return ConversationModel.findOne({ userId: uid, scope })
      .then((conversation) => {
        if (_.isNull(conversation)) {
          this.logger.info('We\'re never talked to this user in this scope before.  Starting a new conversation');
