...
class PluginManager {
  ...
  addPlugin(Plugin) {
    ...
    this.davis.logger.debug(`Loaded plugin: ${pluginInstance.constructor.name}`);
  ...
  loadPlugins(plugins) {
    _.forEach(plugins, (plugin) => {
      try {
        ...
        this.davis.logger.error(e.message);
      }
    });
  }
  ...
  loadCorePlugins() {
    ...
    this.davis.logger.info(`Loading plugins in ${intentsDirectoryPath}`);
    ...
    this.davis.logger.debug(`Found ${coreIntents.length} plugin candidates`);

    this.loadPlugins(coreIntents);
    this.davis.logger.debug(`Loaded ${this.plugins.length} plugins`);
    this.nlp.train();
  }
  ...
  run(exchange) {
    ...
    return BbPromise.reduce(hooks, (__, hook) => hook(exchange), null)
      .then(() => {
        ...
        // { sayPath, show, text }
        this.davis.responseBuilder.build(exchange);
