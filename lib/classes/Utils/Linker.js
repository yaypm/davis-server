...
class Linker {
   ...
   */
  problem(problem) {
    return `${this.home()}/#problems;filter=watched/problemdetails;pid=${problem.id}`;
  }
  ...
  event(problem, event) {
    ...
    if (!modifier) {
      return `${this.home()}/#problems`;
    }

    return `${this.home()}/${modifier}=${event.entityId};gtf=p${problem.id};pid=${problem.id}`;
  }
