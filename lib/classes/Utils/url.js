...
const urlBuilder = {
  ...
  topImpactURL: (user, problem) => {
    ...
    let url = `${user.url}/#problems;filter=watched/problemdetails;pid=${problem.id}`;

    _.forEach(locations, (location) => {
      if (event.entityId.includes(location)) {
        url = `${user.url}/${locations[location]}=${event.entityId};pid=${problem.id}`;
        return;
      }
    });
  ...
  problem: (problem, user) =>
         `${user.url}/#problems;filter=watched/problemdetails;pid=${problem.id}`,

  event: (event, problem, user) => {
    for (const location in locations) {
      if (event.entityId.includes(location)) {
        return `${user.url}/${locations[location]}=${event.entityId};gtf=p_${problem.id};pid=${problem.id}`;
      }
    }

    return `${user.url}/#problems`;
  },
