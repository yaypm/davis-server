...
const urlBuilder = {
  ...
  userActivity: (baseUrl, applicationId, timeRange) => {
    let timeFilter = ';gtf=l_24_HOURS';
    if (timeRange.startTime && timeRange.stopTime) {
      timeFilter = `;gtf=c_${timeRange.startTime}_${timeRange.stopTime}`;
    }
    return `${baseUrl}/#uemapplications/uemappmetrics;uemapplicationId=${applicationId}${timeFilter};visiblepart=sessions`;
  },
