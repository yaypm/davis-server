...
class TimeStamp {
  ...
  toString() {
    return (this.ts === -1) ? 'now' : moment.tz(this.ts, this.tz).calendar().toLowerCase();
  }
}
...
class Card {
  ...
  slack() {
    ...
    if (!_.isNil(this.image_url)) ret.image_url = slackify(this.image_url);
    if (!_.isNil(this.thumb_url)) ret.thumb_url = slackify(this.thumb_url);
...
Card.fromSlack = function (spec) {
  ...
  spec.image_url && card.setImageURL(spec.image_url);
  spec.thumb_url && card.setThumbImageURL(spec.thumb_url);
