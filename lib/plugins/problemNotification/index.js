...
const PROMPTS = {
  OPEN: [
    'Great, the following problem has been resoved',
    'Nice! The following problem has been resoved',
    'Good news, the following problem has been resoved',
  ],
  CLOSED: [
...
class ProblemNotification {
  ...
  notify(exchange, context) {
    const filters = this.davis.filters;
    ...
    const fbTitle = filters.toTitleCase(currentEvent.eventType).replace('A ', '');

    const initialEvent = filters.eventTitle(startingEvent);
    ...
    const details = new VB.Card()
      // Set fallback
      .addFallback(`${fbTitle}\n`)
      .addFallback(`Impacted: ${filters.friendlyEntityName(currentEvent)}\n`)
      .addFallback(`${filters.capitalize(problem.impactLevel)}\n`)
      .addFallback(`Initial Event: ${initialEvent}\n`)
      .addFallback(`Status: ${status}`)
      // Color appropriately
      .setColor(problem.status)
      // Title is a link to the problem
      .addTitle(filters.eventTitle(currentEvent))
      .setLink(filters.buildProblemUrl(problem))
      // Set fields
      .addField(
          'Impacted',
          `${filters.friendlyEntityName(currentEvent)}\n${filters.capitalize(problem.impactLevel)}`,
          true)
      .addField(
          'Initial Event',
          initialEvent,
    ...
    const roots = _.take(_.map(_.filter(problem.rankedEvents, e => e.isRootCause), root =>
      new VB.Card()
        ...
        .addTitle(filters.eventTitle(root))
        .addTitle((root.status === 'OPEN') ? 'Unresolved' : 'Resolved')
        .setLink(filters.buildEventUrl(problem, root))
        .addField(
          'Impacted',
          filters.friendlyEntityName(root),
          true)
        .addField(
          'Time Range',
          new VB.TimeRange(
  ...
  ack(exchange, context) {
    const VB = this.davis.classes.VB;
    const action = context.message.actions[0].name;
    const pid = context.message.actions[0].value;
