...
class PushLink {
  ...
  pushLink(exchange, context) { // eslint-disable-line consistent-return
    ...
    if (!this.davis.server.isSocketConnected(exchange.user)) {
      return exchange.response("Unfortunetly, it appears that you don't have the Voice Navigator (Chrome extension) enabled. For more information, see the davis GitHub wiki on enabling the Voice Navigator.");
    }
