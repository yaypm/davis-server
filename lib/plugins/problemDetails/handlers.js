...
function round(n, sig) {
  ...
  return Math.floor(Math.round(n * mult) / mult);
}
...
function DEFAULT(events, eventType) {
  ...
  if (events[0].affectedUserActionsPerMinute) {
    ...
    show.addField('Affected User Actions/Minute', `${arpm.toFixed(2)}`, true);
  ...
  if (events[0].affectedRequestsPerMinute) {
    ...
    show.addField('Affected Requests/Minute', `${arpm.toFixed(2)}`, true);
