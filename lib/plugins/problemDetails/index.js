  ...
  problemDetails(exchange, context) {
    ...
    if (exchange.isSlackButton) {
      ...
      if (pid === 'no') {
        ...
        return this.reply(exchange, context.message.actions[0].value);
      }
    }
    ...
    if (_.isNumber(choice)) {
      ...
      if (!choice) {
        return exchange.response('Ok').end();
      } else if (problems.length === 1) {
        idx = 0;
      } else {
        return exchange.response("I'm not sure what you mean by that.").end();
      }
