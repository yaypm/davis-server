  ...
  reply(exchange, pid, context) {
    ...
    return this.davis.dynatrace.problemDetails(pid) // eslint-disable-line
      .then((ret) => {
        ...
        say.push(`This is a ${problem.status.toLowerCase()} ${problem.impactLevel.toLowerCase()} level event`);
        ...
        const events = _.map(eventTypes, (e) => {
          ...
          if (handler) {
            ...
            out.show.setLink(th.buildEventUrl(problem, problem.groupedEvents[e][0]));
            return out;
          }
          this.davis.logger.debug(`No handler for ${e}. Using default.`);

          base.show.setLink(th.buildEventUrl(problem, problem.groupedEvents[e][0]));
        ...
        if (affectedApps.length > 0) {
          ...
          say.push(`affecting ${th.humanList(sayApps, 3)}`);
          if (affectedApps.length > 3) {
            say.push(`, and ${affectedApps.length - 3} others`);
          }
        }

        say.push(new VB.TimeRange(problem, exchange.user.timezone), '.');

        if (eventSays.length > 3) {
          say.push('The top three event types were');
          say.push(th.humanList(eventSays, 3), ',');
          say.push('but there were', eventSays.length - 3, 'others as well.')
        } else {
          say.push('This problem was caused by');
          say.push(th.humanList(eventSays, 3), '.');
        }
        ...
        exchange
          .setLinkUrl(th.buildProblemUrl(problem))
          .addContext({
            problem,
          })
