...
class ProblemDetails {
  ...
  problemDetails(exchange, context) {
    ...
    if (exchange.selected) {
      return this.reply(exchange, exchange.selected, context.paging);
    }
    ...
    if (exchange.button) {
      ...
      if (pid === 'no') {
        ...
        return this.reply(exchange, pid, context.paging);
      }
    }
        ...
        const eventCards = _.map(Object.keys(problem.groupedEvents), (e) => {
          ...
          if (handler) {
            return handler(problem.groupedEvents[e], base)
              .setLink(th.buildEventUrl(problem, problem.groupedEvents[e][0]));
          }
