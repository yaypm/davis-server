...
class DavisVersion {
  ...
  davisVersion(exchange) {
    ...
    const templateDir = 'davisVersion/templates/en-us/';

    // const text = `I am currently running Davis Server version ${version}`;
    // const response = { say: text, show: { text  } };
    
    // exchange.greet().response({textString: `You're currently running version {{version}} of Davis!`});

    const templates = this.davis.pluginManager.responseBuilder.getTemplates(templateDir)
