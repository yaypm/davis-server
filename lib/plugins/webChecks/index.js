...
class WebChecks {
        ...
        return this.davis.dynatrace.getFilteredTimeseriesData(exchange, params)
          .then((result) => {
            ...
            for (var point in result.dataPoints) {
              let webCheck = result.dataPoints[point][result.dataPoints[point].length - 1][1];
  ...
  response(exchange, unavailable, webChecksEnabled) {
    ...
    if (unavailable.length > 0) {
      say.push(`There's currently ${unavailable.length} unavailable web check${ (unavailable.length > 1) ? 's' : null }, based on the most recent checks.`);
