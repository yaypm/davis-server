...
class Pager {
  ...
  setPage(exchange, context) {
    ...
    return this.showPage(exchange, exchange.getContext());
  }
    ...
    switch (curPage.length) {
      ...
      case 2:
        if (paging.intent) {
          ...
          buttons
            .addAction('First', 'First', items[i].id)
            .addAction('Second', 'Second', items[i + 1].id)
            .addText(follow);
          out.text += `  ${follow}`;
        }
        break;
      default:
        if (paging.intent) {
          ...
          buttons
            .addAction('First', 'First', items[i].id)
            .addAction('Second', 'Second', items[i + 1].id)
            .addAction('Third', 'Third', items[i + 2].id)
            .addText(follow);
          out.text += `  ${follow}`;
        }
    }
  ...
  selectedBool(exchange, choice, curPage, intent) {
    ...
    return exchange.respond('Which would you like to hear more about?').skipFollowUp();
  }
