      ...
      'userActivity:userActivity': (exchange) => {
        ...
        return this.davis.dynatrace.getFilteredTimeseriesData(exchange, params)
          .then(res => {
            ...
            numApps = entities.length;
            appName = (_.get(nlp, 'app.name') || (numApps === 1)) ? result.entities[entities[0]] : null;
            entity = (appName) ? _.find(this.davis.pluginManager.entities.applications, { name: appName }) : _.find(this.davis.pluginManager.entities.applications, { entityId: _.get(res, 'stats.maxMean.entity') });
            paramsPredict.entity = entity.entityId;
            return this.davis.dynatrace.getFilteredTimeseriesData(exchange, paramsPredict);
          })
          .then(res => {
            if (appName) {
              return this.appResponse(exchange, result, res, entity);
            }
            return this.noAppResponse(exchange, result, res, entity);
          });
      },
    };
  }
