...
class Problem {
  constructor(davis, options) {
    ...
    this.hooks = {
      ...
      'problem:decide': this.decide.bind(this),
  ...
  decide(exchange, context) {
    ...
    if (tense === 'future') {
      exchange.smartEnd();
      exchange.addContext({ templateDir: 'future' });
      return;
    }

    const out = exchange.decide.predict(model, { problems, filtered });
    const templateDir = (tense === 'present') ? path.join('present', out.template) : out.template;
