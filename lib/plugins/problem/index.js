...
class Problem {
  ...
  problemSummary(problem, exchange, context) {
    ...
    if (!isOpen) {
      text.push((appsText.length === 0) ?
        'that did not affect any applications.' :
        `that affected ${th.humanList(appsText)}.`);
    } else  {
      text.push((appsText.length === 0) ?
          'currently ongoing but not affecting any applications.' :
          `that is currently affecting ${th.humanList(appsText)}.`);
    }
