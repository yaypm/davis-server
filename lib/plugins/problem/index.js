...
class Problem {
  ...
  problem(exchange, context) {
    ...
    const problems = _.map(context.problems, problem => this.problemSummary(problem, exchange, context));
    ...
    if (context.problems.length === 0) {
      if (context.tense === 'present') {
        if (app) {
          ...
          leadArr = ['Looks like', problemsLink, 'are occuring right now.'];
        }
        const out = {
      ...
      if (context.tense === 'present') {
        if (app) {
          ...
          leadArr = ['Looks like', problemsLink, 'is occuring right now.'];
        }
      } else if (app) {
        leadArr = ['Looks like', problemsLink, 'affected', app, dateRange, '.'];
      } else {
  ...
  problemSummary(problem, exchange, context) {
    ...
    const appsText = affectedApps.map(app => app.display.visual);

    const affectedEntitiesCount = affectedEntities.length;
    ...
    const show = new VB.Card()
      ...
      .addField('Entities Affected', affectedEntitiesCount, true)
      .slack();

    const text = VB.stringify([
      th.friendlyEventFirstAlias(problem.rankedImpacts[0].eventType),
      (appsText.length === 0) ?
        'affecting no applications.' :
        `affecting ${th.humanList(appsText)}.`,
    ]);
