...
class Alexa {
  ...
  askDavis(req) {
    return BbPromise.resolve()
      .then(() => this._audit(req))
      .then(uid => this.users.validateAlexaUser(uid))
      .then(user => {
        const exchange = new this.Exchange(this.davis, user);
        return exchange.start(this._getRawRequest(req), ALEXA_REQUEST_SOURCE);
      })
  ...
  _getRawRequest(req) {
    ...
    if (requestType === 'LaunchRequest') {
      ...
      throw new error.DError('End session received.');
    } else {
      throw new error.DError(`Received an unknown request type ${requestType}!`);
    }
    return phrase;
  }
