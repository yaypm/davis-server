...
class Alexa {
  ...
  askDavis(req) {
    ...
    return BbPromise.resolve()
      ...
      .then(aid => users.validateAlexaUser(aid))
      .then(user => {
        const exchange = new this.Exchange(this.davis, user);
        return exchange.start(this.getRawRequest(req), ALEXA_REQUEST_SOURCE);
      })
      .then(exchange => exchange.save())
      .then(exchange => (exchange.isFirstInteraction()) ? 'Hello new user!' : 'Welcome back!')
      .then(message => this.formatResponse(message, true))
      .catch(err => this.formatResponse(err.message, true));
  }
  ...
  formatResponse(response, endSession) {
    return {
