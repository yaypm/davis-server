...
class Slack {
  ...
  releaseConversation(bot, scope) {
    ...
    const footer = this.conversations[chanScope].footer;
    if (footer && footer.ts) {
      const message = this.davis.classes.VB.Message.fromSlack(footer.message).setState('sleeping').slack();
  ...
  buttonClick(bot, message) {
    ...
    const working = (button.slow || !button.replace) ?
      replyInteractive(message, noButtons.slack()) :
      BbPromise.resolve();
  ...
  problemNotifications(exchange) {
    ...
    this.logger.info(`Attempting to send out notifications on ${scopes.length} eligible scope(s).`);
  ...
  restartRtm(bot) {
    ...
    const startRTM = BbPromise.promisify(bot.startRTM);
    let attempts = 0;
    const that = this;

    function start () {
      that.logger.debug('attempting to restart rtm');
      setTimeout(() => {
        startRTM()
          .catch((err) => {
            start();
          ...
          .then(() => that.logger.info('A Slack app bot experienced a rtm_close event, but RTM successfully restarted.'))
          .then(() => that.updateCache())
          .catch(() => that.logger.error('An error occurred during slack RTM restart'));
      }, 1000 * attempts);
