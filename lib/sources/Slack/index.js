...
class Slack {
  ...
  interact(bot, message) {
    ...
    postMessage(working)
      .then((res) => {
        this.conversations[chanScope].workingTs = res.ts;
      ...
      .then((res) => {
        ...
        response.ts = this.conversations[chanScope].workingTs;
  ...
  askDavis(message, scope) {
    return this.findUser(message.team, message.user)
      ...
      .then((user) => {
        ...
        return exchange.start(message.text, slackConfig.SLACK_REQUEST_SOURCE, scope);
      })
