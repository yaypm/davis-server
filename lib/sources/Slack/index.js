...
class Slack {
  ...
  start() {
    ...
    if (!this.davis.config.isSlackEnabled()
      ...
      this.logger.warn("Skipping Slack start up because it's already running.");
      return false;
    }
    ...
    this.controller.on('bot_group_joined', (bot, message) => this.botJoinedChannel(bot, message));

    // A bot has joined a channel, say hello
    this.controller.on('bot_channel_joined', (bot, message) => this.botJoinedChannel(bot, message));
      ...
      for (const t in teams) {
        if (teams[t].bot) {
          this.getDavisBotStatus(teams[t].bot.token).then(resp => {
            if (!resp.online) {
              const connectedBot = this.controller.spawn(teams[t]).startRTM((err, bot) => {
                if (err) {
                  this.logger.error('Error connecting bot to Slack:', err.message);
                } else {
              ...
              this.logger.warn('Bot spawn aborted, another instance of Davis that is running is already using this bot');
            }
          });
        }
      }
    ...
    bot.api.channels.list({}, (err, response) => {
      if (response.hasOwnProperty('channels') && response.ok) {
        ...
        for (let i = 0; i < total; i++) {
          ...
          if (channel.is_member) this.davisChannels.push({ name: channel.name, id: channel.id, bot });
        }
      }
    });
  }
