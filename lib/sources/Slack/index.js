...
class Slack {
  ...
  buttonClick(bot, message) {
    ...
    BbPromise.all([
      ...
      .then((exchange) => {
        ...
        response = this.utils.addUsernameAsAuthor(response, `${username} clicked ${action}`);
  ...
  problemNotifications(exchange) {
    ...
    this.logger.info(`Attemping to send out ${scopes.length} notification(s).`);
    _.forEach(scopes, (scope) => {
      const detailedScope = scope.split(':');
      const team = detailedScope[1];
      const channel = detailedScope[2];
      if (_.has(this.davisChannels, { team_id: team, id: channel })) {
        // send message
      }
    });
  }
