...
class Slack {
  ...
  init() {
    ...
    this.event.on('davis.event.problem.*', problem => {
      ...
      const subscribedChannels = this.davis.config.getSlackAlertChannels();

      if (this.davis.config.isSlackAlertsEnabled()) {
        _.each(this.davisChannels, channel => {
          _.some(subscribedChannels, subscribedChannel => {
            if (this.isMatch(subscribedChannel, channel, problem)) {
              if (subscribedChannel.enabled === false) {
                ...
                this.users.getSystemUser()
                  .then(user => {
                    ...
                    user.timezone = subscribedChannel.timezone || user.timezone;
