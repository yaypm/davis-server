...
class SlackConversation {
  ...
  interact(request, convo, bot, user) {
    ...
    let isTimedOut = moment().subtract(slack.INACTIVITY_TIMEOUT, 'seconds').isAfter(this.lastInteractionTime);
    ...
    if ((!this.isDirectMessage && (this.shouldEndSession || isTimedOut)) || 
        this._isOtherUserMentioned(request.text)) {
        ...
        .then(response => {
            ...
            if (!this.isDirectMessage) {
                ...
                if (response.attachments.length > 1 && response.attachments[response.attachments.length - 2].actions) {
                                ...
                                callbacks.push(
                                    {
                                        ...
                                        callback: (req, convo) => {
                                            ...
                                            if (req) {
                                                this.interact(req, convo, bot, user);
                                            } else {
  ...
  updateListeningStateFooter(convo, bot, responded) {
          ...
          let channel = convo.source_message.channel;
