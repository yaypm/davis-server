...
class SlackConversation {
    ...
    if ((!this.isDirectMessage && (this.shouldEndSession)) ||
      ...
      this.askDavis(request, user)
        .then((res) => {
          ...
          if (res.filtered) {
            response.attachments[0].author_icon = 'https://s3.amazonaws.com/dynatrace-davis/assets/images/filter_blue_15_15.png';
          }
  ...
  stripEmojis(str) {
    ...
    if (result === '') {
      result = 'hey davis';
      this.logger.warn('Error: Slack stripped emojis and was going to send an empty string');
    }
