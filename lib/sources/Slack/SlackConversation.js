...
class SlackConversation {
  ...
  interact(request, convo, bot, user) {
    if (request.callback_id) { // user clicked a button
      this.removeButtons(bot, convo, request.original_message, request.action_ts);
      if (request.callback_id.startsWith('_proactive_')) return;
    }
  ...
  removeButtons(bot, convo, message, ts) {
    const newMessage = this.davis.utils.Slack.removeButtons(message);
  ...
  askDavis(req, member) {
    ...
    return BbPromise.resolve()
      ...
      .then((user) => {
        const scope = `${slack.SLACK_REQUEST_SOURCE}:${req.team}:${req.channel}`;
