...
class Davis {
  ...
  migrate() {
    const out = child_process.spawnSync('npm', ['run', 'migrate', '--', '-d', this.config.getMongoDBConnectionString(), 'up', '--es6', '--autosync']);
    this.logger.debug(String(out.stdout));
  }

  addCron(cronTime, onTick, name) {
