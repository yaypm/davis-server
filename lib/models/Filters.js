...
FilterSchema.methods.problemMatch = function (problem) {
  ...
  if (this.entityID.length > 0 || this.excludeEventType.length > 0) {
    for (const impact of problem.rankedImpacts) {
      if (this.entityID.length > 0 && this.entityID.indexOf(impact.entityId) === -1) return false;
      if (this.excludeEventType.length > 0 && this.excludeEventType.indexOf(impact.entityId) !== -1) return false;
    }
  }
