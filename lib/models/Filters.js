...
FilterSchema.index({ name: 1, scope: 1 }, { unique: [true, 'This name is already being used!'] });
  ...
  if (this.entityID.length > 0 || this.excludeEventType.length > 0) {
    ...
    for (const event of events) {
      if (this.entityID.length > 0) {
        if (!hasMatchingEntityID && this.entityID.indexOf(event.entityId) !== -1) {
    ...
    if (this.entityID.length > 0 && !hasMatchingEntityID) {
      console.log("The problem didn't contain a matching entity ID.");
      return false;
    }
  }

  console.log(`Problem ${problem.id} matches the filter.`);
  return true;
};
