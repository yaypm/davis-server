...
function getAliases(name) {
    ...
    for (category in filteredAliases) {
        ...
        filteredAliases[category].forEach( function (als) {
            ...
            var template = `<table id="${als._id}-table" class="alias"><tr><td class="property" style="font-weight: bold;">Name</td>
            <td class="value"><input type="text" id="${als._id}-name" class="textInput" value="${als.name}" readonly><div class="${noAliasesClass}no-aliases-wrapper">
                <div class="no-aliases-text">No<br>Aliases</div><div class="no-aliases-triangle">
            </div></div></td></tr>
            <tr><td class="property">Visual</td><td class="value"><input type="text" id="${als._id}-visual" class="textInput" value="${als.display.visual}"></td></tr>
            <tr><td class="property">Audible</td><td class="value"><input type="text" id="${als._id}-audible" class="textInput" value="${als.display.audible}"></td></tr>
            <tr><td class="property">Aliases</td></tr><tr><td class="aliases" colspan="2">`;
            als.aliases.forEach( function (alias, index) {
                template += `<div class="wrapper"><input type="text" class="textInput ${als._id}-alias" value="${alias}"><div class="comma">,</div></div>`;
            });
            ...
            </td></tr><tr><td colspan="2"><input type="button" id="${als._id}-save-button" value="Save" onclick="saveAliasesTile('${als._id}','${category}');">`;
...
function getAliasData() {
    ...
    fetch('/api/v1/system/aliases', options)
...
function saveAliasesTile(alsId, category) {
    var aliasValues = $(`.${alsId}-alias`).val();
    if (typeof aliasValues === 'string') {
    ...
    fetch(`/api/v1/system/aliases/${category}/${alsId}`, options)
        .then(function (response) {
